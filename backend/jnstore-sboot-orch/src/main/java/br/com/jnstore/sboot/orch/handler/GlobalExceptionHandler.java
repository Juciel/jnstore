package br.com.jnstore.sboot.orch.handler;
...
public class GlobalExceptionHandler {
    ...
    public ResponseEntity<ErroRepresetation> handleFeignException(FeignException ex, HttpServletRequest request) {
        final HttpStatus finalStatus = Optional.ofNullable(HttpStatus.resolve(ex.status()))
                .orElse(HttpStatus.INTERNAL_SERVER_ERROR);

        ErroRepresetation erro = ex.responseBody()
                .flatMap(byteBuffer -> {
                    try {
                        return Optional.of(objectMapper.readValue(byteBuffer.array(), ErroRepresetation.class));
                    } catch (IOException e) {
                        log.warn("Não foi possível deserializar o corpo do erro do Feign.", e);
                        return Optional.empty();
                    }
                })
                .orElseGet(() -> {
                    ErroRepresetation erroPadrao = new ErroRepresetation();
                    erroPadrao.setTimestamp(OffsetDateTime.now());
                    erroPadrao.setStatus(finalStatus.value());
                    erroPadrao.setError(finalStatus.getReasonPhrase());
                    erroPadrao.setMessage(ex.getMessage()); // Fallback para a mensagem da exceção
                    erroPadrao.setPath(request.getRequestURI());
                    return erroPadrao;
                });

        log.warn("Erro de comunicação com microsserviço (Feign): Status {}, Erro: {}", finalStatus, erro);
        return new ResponseEntity<>(erro, finalStatus);
    }
